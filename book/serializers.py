...
class BookSerializer(serializers.HyperlinkedModelSerializer):
    ...
    )  # Allow selection of author by ID during POST
    ...
    class Meta:
        ...
        fields = [
            ...
            "author_url",
            "author",
        ]  # Added 'author_url' to fields
        validators = [
            UniqueTogetherValidator(
                queryset=Book.objects.all(), fields=["author", "title"]
            )
        ]
    ...
    def validate_publication_date(self, value):
        if value < datetime(1970, 1, 1).date():
            raise serializers.ValidationError(
                "The publication date must be after 1970."
            )
        return value
